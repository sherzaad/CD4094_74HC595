...
const uint8_t latchPin = 4; // ST_CP(pin 12)	|	ST_CP(pin 1)
const uint8_t clockPin = 3; // SH_CP(pin 11)	|	SH_CP(pin 3)
const uint8_t dataPin = 2;  // DS(pin 14)		|	DS(pin 2)
...
void loop() {
  ...
  for (int numberToDisplay = 0; numberToDisplay < 256; numberToDisplay++) {
    ...
    // Shift out the bits
    if (ShiftOut == LSBFIRST) {
      for (int i = 0; i < 8; i++) {
        digitalWrite(dataPin, (numberToDisplay & (1 << i)));
